...
class SLiteCSDNet(nn.Module):
    def __init__(self):
        ...
        self.downsample_1 = nn.MaxPool2d(8)

        self.attention_conv7_2 = nn.Conv2d(self.channels + 2, self.channels, 3, padding=p)
    ...
    def forward(self, input, gray):
        ...
        g_conv6 = F.upsample(torch.cat([gray_4, conv4], 1), scale_factor=2, mode='bilinear')
        # conv3 = conv3 * gray_3 if self.opt.self_attention else conv3
        g_up7 = g_conv6
        g_conv7 = self.attention_bn7_2(self.attention_LReLU7_2(self.attention_conv7_2(g_up7)))
        self.gray_3 = self.attention_sigmoid_3(g_conv7)

        g_conv7 = F.upsample(g_conv7, scale_factor=2, mode='bilinear')
        # conv2 = conv2 * gray_2 if self.opt.self_attention else conv2
        g_up8 = g_conv7
        g_conv8 = self.attention_bn8_2(self.attention_LReLU8_2(self.attention_conv8_2(g_up8)))
        self.gray_2 = self.attention_sigmoid_2(g_conv8)

        g_conv8 = F.upsample(g_conv8, scale_factor=2, mode='bilinear')
        # conv1 = conv1 * gray if self.opt.self_attention else conv1
        g_up9 = g_conv8
        g_conv9 = self.attention_LReLU9_2(self.attention_conv9_2(g_up9))
        self.gray_1 = self.attention_sigmoid_1(g_conv9)

        g_latent = self.attention_conv10(g_conv9)
        self.gray = self.attention_sigmoid(g_latent)
