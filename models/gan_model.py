...
class GanModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            if self.opt.attention_D:
                self.netD_Atten = networks.define_D(opt.input_nc, opt.ndf,
                                                    'attention_D',
                                                    opt.n_layers_patchD, opt.norm, use_sigmoid, self.gpu_ids, True)
        ...
        if self.isTrain:
            ...
            self.illu_smooth_loss = networks.max_weighted_smooth_loss()
    ...
    def backward_D_Atten(self):
        use_ragan = True
        fake_B = self.fake_B_pool.query(self.fake_B)
        fake_B = self.fake_B
        atten = 1. - self.input_A_gray
        pred_real = self.netD_Atten.forward(self.real_B, atten)
        pred_fake = self.netD_Atten.forward(self.fake_B, atten)
        if self.opt.use_wgan:
            loss_D_real = pred_real.mean()
            loss_D_fake = pred_fake.mean()
            loss_D = loss_D_fake - loss_D_real + self.criterionGAN.calc_gradient_penalty(netD,
            ...
            loss_D = (loss_D_real + loss_D_fake) * 0.5
        # loss_D.backward()
        self.loss_D_Atten = loss_D
        self.loss_D_Atten.backward()
    ...
    def forward(self):
        ...
        self.fake_B, self.latent_real_A, self.gray = self.netG_A.forward(self.real_img, self.real_A_gray)
    ...
    def backward_G(self, epoch):
        ...
        self.loss_G += self.gray_smooth_loss
        # self.loss_G += self.real_smooth_loss
    ...
    def optimize_parameters(self, epoch):
        ...
        # if not self.opt.attention_D:
        #     self.optimizer_D_A.step()
        # else:
        #     self.optimizer_D_Atten.zero_grad()
        #     self.backward_D_Atten()
        #     self.optimizer_D_A.step()
        #     self.optimizer_D_Atten.step()

    def get_current_errors(self, epoch):
        D_A = self.loss_D_A.data[0]
        D_P = self.loss_D_P.data[0] if self.opt.patchD else 0
        D_Atten = self.loss_D_Atten.data[0] if self.opt.attention_D else 0
        G_A = self.loss_G_A.data[0]
        if self.opt.vgg > 0:
            vgg = self.loss_vgg_b.data[0] / self.opt.vgg if self.opt.vgg > 0 else 0
            return OrderedDict([('D_A', D_A), ('G_A', G_A), ("vgg", vgg), ("D_P", D_P), ("D_Atten", D_Atten)])
        elif self.opt.fcn > 0:
            fcn = self.loss_fcn_b.data[0] / self.opt.fcn if self.opt.fcn > 0 else 0
            return OrderedDict([('D_A', D_A), ('G_A', G_A), ("fcn", fcn), ("D_P", D_P), ("D_Atten", D_Atten)])
